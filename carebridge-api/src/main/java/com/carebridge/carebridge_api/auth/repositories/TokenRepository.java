...
public interface TokenRepository extends JpaRepository<Token, Long> {
    ...
    @Query(value = "SELECT * FROM t_token mt WHERE mt.email = ?1 AND mt.used_for = ?2 AND mt.created_on = ((SELECT MAX(created_on) FROM t_token WHERE email = ?1 AND used_for = ?2))", nativeQuery = true)
