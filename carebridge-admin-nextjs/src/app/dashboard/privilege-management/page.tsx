    ...
    <ResourceView<User>
      ...
      formBuilder={
        <form onSubmit={handleSubmit(handleSubmitUserForm)} noValidate className="flex flex-col gap-4">
          <Controller name="fullName" control={control} render={({ field }) => <AppTextField {...field} variant="outlined" sizes="small" label="Fullname" helperText={errors.fullName?.message || 'Enter your name'} isError={!!errors.fullName} isRequired />} />
          <Controller name="email" control={control} render={({ field }) => <AppTextField {...field} variant="outlined" sizes="small" label="Email" helperText={errors.email?.message || 'Enter your email'} isError={!!errors.email} isRequired />} />
          <Controller name="address" control={control} render={({ field }) => <AppTextField {...field} variant="outlined" multiline sizes="small" label="Address" helperText={errors.address?.message || 'Enter your address'} isRequired />} />
          {pageState.dialogMode === 'view' && <Controller name="password" control={control} render={({ field }) => <AppTextField {...field} variant="outlined" sizes="small" label="Password" type="password" helperText={errors.password?.message || 'Enter your password'} isRequired isError={!!errors.password} isDisabled={pageState.dialogMode === 'view'} />} />}
        </form>
      }
    />
  );
